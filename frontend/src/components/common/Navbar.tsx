...
const Navbar = ({ onSearch, searchText, setOpenSidebar }: Props) => {
  ...
  // Local state to prevent mobile soft keyboard IME cursor resets (KGF -> FGK fix)
    ...
    <>
      ...
      <nav className="w-full flex px-3 sm:px-6 lg:px-8 py-2.5 sm:py-4 bg-white dark:bg-zinc-950 border-b border-gray-200 dark:border-zinc-900 text-gray-900 dark:text-white items-center justify-between sticky top-0 z-50 transition-all duration-500">
        <div className="flex items-center flex-1 min-w-0">
          ...
          <div className="flex-1 flex items-center mx-2 sm:mx-8 relative max-w-xl min-w-0">
            <SearchIcon size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 sm:size-[20px]" />
            <Input
              value={localSearch}
              ...
              autoCorrect="off"
              spellCheck={false}
              className="pl-8 sm:pl-10 pr-8 text-xs sm:text-base h-8 sm:h-10 w-full rounded-xl bg-gray-100 dark:bg-zinc-900 border-gray-200 dark:border-zinc-800 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-zinc-500 outline-none focus:ring-2 focus:ring-red-600 transition-all"
            />
            {localSearch && (
              <X
                size={16}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 cursor-pointer hover:text-red-500 sm:size-[20px]"
                onClick={() => {
                  setLocalSearch("");
                  onSearch("");
                }}
