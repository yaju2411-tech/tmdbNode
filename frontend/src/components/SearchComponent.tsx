...
const GlobalSearch = ({ searchText,user,watchlist }: Props) => {
      ...
      <div className="overflow-x-hidden flex flex-col max-h-[70vh] overflow-y-auto scrollbar-hide">
        {safeContent?.map((item: any) => (
          <div
            ...
            onClick={() =>{navigate(item.media_type === "movie" ? `/app/movieDetail/${item.id}` : `/app/tvDetail/${item.id}`);}
            }
            className="flex items-center justify-between min-w-md cursor-pointer py-2"
          >
